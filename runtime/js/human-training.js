(function (root, factory) {
  const deps = (typeof module === 'object' && module.exports)
    ? {
        ...
        simCore: require('./sim-core.js'),
        controllers: require('./controllers.js')
      }
    : {
        config: root && root.WavePong && root.WavePong.CONFIG,
        simCore: root && root.WavePong && root.WavePong.SimCore,
        controllers: root && root.WavePong && root.WavePong.Controllers
      };
  ...
  if (root) {
    ...
    root.WavePong.HumanTraining = api;
  }
})(typeof globalThis !== 'undefined' ? globalThis : this, function (deps) {
  const config = deps && deps.config;
  const simCore = deps && deps.simCore;
  const controllers = deps && deps.controllers;

  if (!config) throw new Error('Wave Pong config missing. Load js/config.js before js/human-training.js.');
  if (!simCore) throw new Error('Wave Pong sim core missing. Load js/sim-core.js before js/human-training.js.');
  if (!controllers) throw new Error('Wave Pong controllers missing. Load js/controllers.js before js/human-training.js.');
  ...
  function normalizeDatasetPayload(raw) {
    ...
    if (raw.schema === EXPORT_SCHEMA) {
      ...
      return {
        ...
        summary: buildDatasetSummary(normalized.sessions)
      };
    }
    if (raw.schema && raw.schema !== DATASET_SCHEMA) {
      throw new Error(`Unsupported human training dataset schema: ${raw.schema}`);
    }
    const sessions = (Array.isArray(raw.sessions) ? raw.sessions : []).map(normalizeTrainingSession).filter(Boolean);
    return {
      schema: DATASET_SCHEMA,
      importedAt: raw.importedAt || null,
      sourceSchema: raw.sourceSchema || EXPORT_SCHEMA,
      sourceFile: raw.sourceFile || null,
      sessions,
      summary: raw.summary ? raw.summary : buildDatasetSummary(sessions)
    };
  }
  ...
  function computePositiveWeights(samples, outputSize, maxPositiveWeight = 24) {
    const positiveCounts = new Array(outputSize).fill(0);
    for (const sample of samples) {
      ...
      for (let i = 0; i < outputSize; i += 1) {
        positiveCounts[i] += targets[i] ? 1 : 0;
      }
    }
    return positiveCounts.map((count) => clamp((samples.length - count) / Math.max(1, count), 1, maxPositiveWeight));
  }
  ...
  function trainNetworkWithBce(network, samples, options = {}) {
    ...
    const maxPositiveWeight = Math.max(1, Number(options.maxPositiveWeight) || 24);
    const random = options.random || Math.random;
    const positiveWeights = Array.isArray(options.positiveWeights) && options.positiveWeights.length
      ? options.positiveWeights.map((value) => Math.max(1, Number(value) || 1))
      : computePositiveWeights(samples, outputSize, maxPositiveWeight);
    let totalLoss = 0;
      ...
      for (let batchStart = 0; batchStart < shuffled.length; batchStart += batchSize) {
        ...
        for (const sample of batch) {
          ...
          for (let i = 0; i < outputSize; i += 1) {
            ...
            const positiveWeight = positiveWeights[i] || 1;
            totalLoss += -((target ? positiveWeight : 1) * target * Math.log(probability) + (1 - target) * Math.log(1 - probability));
            lossSampleCount += 1;
            deltas[i] = target ? (probability - 1) * positiveWeight : probability;
          }
